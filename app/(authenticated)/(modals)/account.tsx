...
const Page = () => {
  ...
  const onSaveUser = async () => {
    try {
      await user?.update({ firstName: 'karim', lastName: 'Salim' });
      setEdit(false);
    } catch (error) {
      console.error(error);
    } finally {
    ...
    >
      <View style={{ alignItems: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          ...
          {edit && (
            <View style={styles.editRow}>
              <TextInput placeholder="First Name" value={'Kairm'} style={[styles.inputField]} />
              <TextInput placeholder="Last Name" value={'Salim'} style={[styles.inputField]} />
